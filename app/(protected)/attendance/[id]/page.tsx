...
import { decorateAttendanceSessions } from "@/lib/attendance/ledger";
...
function timekeeperLine(name: string | null, id: string | null) {
...
export default async function AttendanceDetailPage({
  ...
  const { data } = await supabase
    ...
    .eq("id", id)
    .maybeSingle();
  const storedSession = data as unknown as AttendanceSessionRow | null;
  if (!storedSession) notFound();
  const [session] = await decorateAttendanceSessions(supabase, [storedSession]);
    ...
    <section>
      <header className="page-header">
        <div>
          ...
          <p>
            {session.site_name_snapshot} · {session.work_date}
          </p>
        </div>
        <span className={`ledger-status ledger-status--${session.status}`}>
          {session.status}
        </span>
      </header>
            ...
            <div>
              ...
              <dd>
                {evidenceLine(
                  ...
                  session.check_in_accuracy_metres,
                )}
              </dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd>
                {evidenceLine(
                  ...
                  session.check_out_accuracy_metres,
                )}
              </dd>
            </div>
            <div>
